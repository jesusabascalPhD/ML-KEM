...
from .codec import byte_decode, byte_encode, compress_poly, decode_vec, decompress_poly, encode_vec
from .constants import PARAMS, N
from .hash_utils import PRF, G, H, J
from .poly import mat_vec_mul, ntt, ntt_inv, poly_add, poly_sub, vec_add, vec_dot
from .sampling import sample_cbd, sample_ntt
...
class MLKEM:
    ...
    def __init__(self, security_level: int = 768) -> None:
        if security_level not in PARAMS:
            raise ValueError(
                f"Nivel de seguridad inválido: {security_level}. Opciones válidas: {sorted(PARAMS)}"
            )
    ...
    def _generate_matrix(self, rho: bytes, transpose: bool = False):
        ...
        k = self.k
        return [
            [sample_ntt(rho, j if transpose else i, i if transpose else j) for j in range(k)]
            for i in range(k)
        ]
    ...
    def _pke_encrypt(self, ek: bytes, m: bytes, r: bytes) -> bytes:
        ...
        # v = tᵀ·y + e2 + mu
        v = poly_add(poly_add(ntt_inv(vec_dot(t_hat, y_hat)), e2), mu)
